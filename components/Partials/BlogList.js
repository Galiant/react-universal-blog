...
export default class BlogList extends Component {
  ...
  render() {
    ...
    if (articles && item_num <= articles.length) {
      load_more = (
        <div>
          <button className="btn btn-default center-block" onClick={
            this.props.getMoreArticles.bind(this)}>
            {show_more_text}
          </button>
        </div>
      )
    ...
    let articles_html = articles.map((article) => {
      ...
      let created = (date_obj.getMonth() + 1) + '/' + date_obj.getDate() + '/' + date_obj.getUTCFullYear()
        ...
        <div key={'key-' + article.slug}>
          <div className="post-preview">
            ...
            <p className="post-meta">
              Posted by <a href="https://cosmicjs.com" target="_blank">Cosmic JS</a> on {created}
            </p>
          </div>
          <hr />
        </div>
